...
export async function POST(request: Request) {
  try {
    ...
    const { walletAddress } = ScanSchema.parse(body);

    console.log(`🔍 Scanning positions for ${walletAddress} using authenticated API...`);
    ...
    // Fetch reward configuration from CLOB API
    console.log('Fetching reward configuration from Polymarket...');
    const rewardConfigResponse = await fetch('https://clob.polymarket.com/sampling-simplified-markets');
    if (!rewardConfigResponse.ok) {
      throw new Error(`Failed to fetch reward config: ${rewardConfigResponse.statusText}`);
    }
    const rewardConfigData = await rewardConfigResponse.json();
    ...
    console.log(`\n🔍 Fetching market details (including rewards)...`);
    const marketDetailsMap = new Map();

    for (const marketId of ordersByMarket.keys()) {
      try {
        console.log(`   Fetching details for market ${marketId.substring(0, 20)}...`);
        const marketResponse = await fetch(`https://clob.polymarket.com/markets/${marketId}`);

        if (marketResponse.ok) {
          const marketData = await marketResponse.json();

          // Calculate daily reward if present
          let dailyReward = 0;
          ...
          console.log(`   ✅ Market:`, {
            ...
            hasRewards: !!marketData.rewards,
            dailyReward: dailyReward > 0 ? `$${dailyReward}` : 'none'
          });
          ...
          console.log(`   ⚠️  Failed to fetch market details: ${marketResponse.status}`);
        }
      } catch (error) {
